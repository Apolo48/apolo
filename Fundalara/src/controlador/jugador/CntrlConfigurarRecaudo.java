...
public class CntrlConfigurarRecaudo extends GenericForwardComposer {
	...
	private Button btnModificar,btnEliminar, btnGuardar, btnCancelar, btnSalir;
	...
	public void doAfterCompose(Component comp) throws Exception {
		...
		comp.setVariable("controller", this, true);  //Hacemos visible el modelo para el databinder
		formulario = comp; 	
	...
	public void onClick$btnGuardar() throws InterruptedException {
		listrecaudos=servicioRecaudoPorProceso.listar();
		if(cmbRecaudo.getSelectedIndex()!=-1 && cmbProceso.getSelectedIndex()!=-1 
		 && cmbAccion.getSelectedIndex()!=-1 && cmbImportancia.getSelectedIndex()!=-1 ){
		for (int i = 0; i < listrecaudos.size(); i++) {
			if(listrecaudos.get(i).getDatoBasicoByCodigoProceso().getNombre().equals(accion.getNombre()) 
			   && listrecaudos.get(i).getDatoBasicoByCodigoDocumento().getNombre().equals(recaudo.getNombre())
			   && listrecaudos.get(i).getDatoBasicoByCodigoImportancia().getNombre().equals(importancia.getNombre())){
				Messagebox.show("El Recaudo ya se encuentra agregado", "GUARDAR", Messagebox.OK, Messagebox.EXCLAMATION);
				return;
			}
		}
		recaudoporguardar= new RecaudoPorProceso();
		recaudoporguardar.setCodigoRecaudoPorProceso(servicioRecaudoPorProceso.listar().size()+1);
		...
		limpiar();
		Messagebox.show("El registro se ha agregado satisfactoriamente", "GUARDAR", Messagebox.OK, Messagebox.INFORMATION);
		
		}
		else{
			Messagebox.show("Complete los datos para poder guardar el recaudo", "GUARDAR", Messagebox.OK, Messagebox.EXCLAMATION);
		}
	}
	...
	public void onClick$btnEliminar() throws InterruptedException {
		Messagebox.show("Esta seguro que Desea Desactivar el Recaudo?", "ELIMINAR", Messagebox.YES|Messagebox.NO, Messagebox.QUESTION,
			     new EventListener() {
			       public void onEvent(Event evt) {
			         switch (((Integer)evt.getData()).intValue()) {
